...
def load_dataset(train_file_path: str, test_file_path: str) -> pd.DataFrame:
    ...
    df1["Label"] = df1.apply(lambda row: "Train", axis=1)
    df2 = pd.read_csv(test_file_path)
    df2["Label"] = df2.apply(lambda row: "Test", axis=1)
...
def get_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    arr = numpy.zeros((df.shape[1], 2))
    columns = ['Total', 'Percent']
    missing = pd.DataFrame(data=arr, index=df.columns, columns=columns)
    row_count = df.shape[0]

    missing['Total'] = [df[col].isnull().sum() for col in missing.index]
    missing['Percent'] = missing.Total / row_count * 100
    missing = missing.sort_values('Total', axis=0, ascending=False)
    return missing
...
def get_survived_per_class(df: pd.DataFrame,
    ...
    grouped = numpy.round(df.groupby(group_by_column_name)['Survived'].sum() / df.groupby(group_by_column_name)['Survived'].count(), 2)
    grouped.columns = ['Survived']
    grouped = pd.DataFrame(grouped, index=grouped.index, columns=grouped.columns)
    grouped = grouped.sort_values('Survived', axis=0, ascending=False)
    return grouped.Survived
...
def get_outliers(df: pd.DataFrame) -> (int, pd.DataFrame):
   ...
   # iqr = q3 - q1
    passanger_col = 'PassengerId'

    mask = ~((2.5 * q1 - 1.5*q3 < df['Fare']) & (2.5 * q3 - 1.5*q1 > df['Fare']))
    outliers = df[mask]
    return (outliers.shape[0] - 1, outliers)
...
def create_new_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df2 = df.copy()
    df2["Fare_scaled"] = (df['Fare'] - df['Fare'].mean())/ df['Fare'].std()
    df2['Age_log'] = numpy.log(df2['Age'])
    df2['Sex'] = df2["Sex"].replace(to_replace={'male': 0, 'female': 1})
    return df2
...
def determine_survival(df: pd.DataFrame, n_interval: int, age: float,
    ...
    minimum = float(df['Age'].min())
    maximum = float(df['Age'].max())
    step = float(maximum-minimum)/n_interval
    numbering = range(n_interval)
    intervals = [minimum + x * step for x in numbering]
    higher = 0
    intervals[0] -= 0.001
    ...
    if higher == 0:
        return numpy.nan
    mask1 = higher >= df['Age']
    mask2 = higher - step < df['Age']
    mask3 = df['Sex'] == sex
    mask4 = ~df['Survived'].isnull()
    mask = (mask1 & mask2 & mask3 & mask4)
    suma = df.loc[mask]['Survived'].sum()
    count = mask.sum()
    if count == 0:
        return numpy.nan
    return suma/count
